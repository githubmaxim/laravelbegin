...
//        $str = 'some thing2';
//        $result = Cache::remember('example',60*60, function () use ($str) { //используется вместо "get()" и "put()", т.к. вначале проверяет наличие записи с таким ключем в БД DB1
//                                                                           //если находит, то возвращает значение такого ключа. Если не находит, то возвращает значение
//                                                                            //указанное после "return" значение + создает в БД DB1 строку с таким ключем и значением указанным после "return".
//                                                                            //Кроме того тут:
...
// Один из вариантов, как сохранять пришедшие данные в БД + Кэш(для поддержания его в актуальном состоянии)
